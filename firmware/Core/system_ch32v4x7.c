...
/* tfpcmcia: SYSCLK = HCLK = PSRAM clock = 200 MHz from HSI (no crystal on
 * the board). Keeps the core at its spec maximum and the PSRAM at the
 * 200 MHz limit that standard (non-good-lot) CH32V467 silicon supports. */
#define SYSCLK_200MHz_HCLK_200MHz_HSI  200000000
...
#elif defined SYSCLK_200MHz_HCLK_200MHz_HSI
uint32_t SystemClock         = SYSCLK_200MHz_HCLK_200MHz_HSI;        /* System Clock Frequency */
uint32_t SystemCoreClock     = SYSCLK_200MHz_HCLK_200MHz_HSI;        /* HPRE = DIV1: HCLK = SYSCLK */
...
#elif defined SYSCLK_200MHz_HCLK_200MHz_HSI
static void SetSYSCLK_200MHz_HCLK_200MHz_HSI( void );
#elif defined SYSCLK_120MHz_HCLK_60MHz_HSI
...
#elif defined SYSCLK_200MHz_HCLK_200MHz_HSI
    SetSYSCLK_200MHz_HCLK_200MHz_HSI();
#elif defined SYSCLK_120MHz_HCLK_60MHz_HSI
...
/*********************************************************************
 * @fn      SetSYSCLK_200MHz_HCLK_200MHz_HSI
 *
 * @brief   SYSCLK = HSI(20MHz) * 10 = 200 MHz, HPRE = DIV1 so
 *          HCLK = 200 MHz and the PSRAM clock (SYSCLK domain) is
 *          200 MHz. Modelled on SetSYSCLK_240MHz_HCLK_120MHz_HSI.
 *
 * @return  none
 */
static void SetSYSCLK_200MHz_HCLK_200MHz_HSI(void)
{
    ...
    /* PLL configuration: SYSCLK = HSI * 10 / 1 = 200 MHz */
    RCC->CFGR0 |= (uint32_t)(RCC_PLLSRC_HSI | RCC_PLLMULL10);
